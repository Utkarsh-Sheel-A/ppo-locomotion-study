"""
Standalone final evaluation for a trained Ant PPO model.
Picks the run by --lr, matching train_ant_ppo.py's naming (ant_ppo_lr<value>).

    python evaluate_ant.py --lr 3e-4
    python evaluate_ant.py --lr 1e-4
...
def main(learning_rate: float, checkpoint: str):
    lr_tag = f"lr{learning_rate:.0e}"
    run_name = f"ant_ppo_{lr_tag}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr", type=float, default=3e-4, help="which lr run to evaluate, e.g. 3e-4 or 1e-4")
    parser.add_argument("--checkpoint", type=str, default="best_model", choices=["best_model", "final_model"])
    args = parser.parse_args()
    main(learning_rate=args.lr, checkpoint=args.checkpoint)
